...
class _PorterAlgorithm:
    ...
    def _step1b(self):
        if self.word.endswith("eed"):
            ...
            elif self.word[-2] == self.word[-1] and self.word[-1] not in [
                "l",
                "s",
                "z",
            ]:
    ...
    def _step4(self):
        ...
        if self._measure(self.word[:-4]) > 1 and self.word[-4:] in [
            "ance",
            "ence",
            "able",
            "ible",
        ...
        if self._measure(self.word[:-3]) > 1 and (
            self.word[-3:] in ["ant", "ent", "ism", "ate", "iti", "ous", "ive", "ize"]
            or self.word.endswith("ion")
            and self.word[-4] in ["s", "t"]
        ):
            self.word = self.word[:-3]

        if self._measure(self.word[:-2]) > 1 and self.word[-2:] in [
            "al",
            "er",
            "ic",
            "ou",
    ...
    def _step5a(self):
        if self.word[-1] == "e" and (
            ...
            or (
                self._measure(self.word[:-1]) == 1
                and not self._ends_cvc(self.word[:-1])
            )
        ):
            self.word = self.word[:-1]
